...
void setup(){
  ...
  Serial.println("Change speed is \"speed=\"");
}

int scroll_speed = 50;

char scr_str[70] = "KIM JUN  ";
...
void loop(){
  ...
  if(Serial.available() > 0){
    ...
    char Str[8];
    getStr.toCharArray(Str, 7); 
    if(!strcmp(Str,"speed=")){
      Serial.println("What speed?");

      while(1){
        if(Serial.available() > 0){
          scroll_speed = Serial.readStringUntil('\n').toInt();
      ...
      Serial.println("Copy!");
      getStr.toCharArray( scr_str, 70);
      strcat(scr_str,"     ");
    }
    
  }
  printStringWithShift(scr_str, scroll_speed);
//  delay(100);
//  m.shiftLeft(false, true);
  
  /*
  // print the active sentences
 printStringWithShift(string1, 20);
 printStringWithShift(string2, 20);
 printStringWithShift(string3, 20);
 printStringWithShift(string4, 20);
